    ...
    void Update() {
      for (int i = 0; i < _pinchDetectors.Length; i++) {
        ...
        if (detector.DidStartPinch)
                {
                    lastPosition = _pinchDetectors[1].Position;
                    print("Detector Start Position is : " + _pinchDetectors[1].Position.x + " " + _pinchDetectors[1].Position.y + " " + _pinchDetectors[1].Position.z);
                    drawState.BeginNewLine();
        }

        if (detector.DidEndPinch)
                {
                    drawDistance = drawDistance + Vector3.Distance(lastPosition, _pinchDetectors[1].Position);
                    Debug.LogWarning("distancePinch ist laut meiner besch...eidenen Berechnung: " + drawDistance);
                    //drawDistance = 0;
